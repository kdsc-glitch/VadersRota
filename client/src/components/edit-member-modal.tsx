...
export function EditMemberModal({ isOpen, onClose, member }: EditMemberModalProps) {
  ...
  const [holidayStartDate, setHolidayStartDate] = useState<Date>();
  const [holidayEndDate, setHolidayEndDate] = useState<Date>();
  ...
  useEffect(() => {
    if (member) {
      ...
      if (member.holidayStart) {
        setHolidayStartDate(new Date(member.holidayStart));
      }
      if (member.holidayEnd) {
        setHolidayEndDate(new Date(member.holidayEnd));
      }
    }
  }, [member, form]);
